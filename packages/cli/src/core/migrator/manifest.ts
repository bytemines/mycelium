...
import { isLikelySecret } from "../secret-detector.js";
...
export interface MigrationSecretExtraction {
  /** Number of secrets extracted */
  count: number;
  /** Key names that were extracted */
  keys: string[];
}

export function serializeMcpsYaml(
  mcps: ScannedMcp[],
  options?: { extractSecrets?: boolean; secretsOut?: Record<string, string> }
): string {
  const shouldExtract = options?.extractSecrets !== false;
  const secretsOut = options?.secretsOut ?? {};
  ...
  for (const mcp of mcps) {
    ...
    if (mcp.config.env) {
      ...
      for (const [key, val] of Object.entries(mcp.config.env)) {
        ...
        if (shouldExtract && isLikelySecret(key, strVal)) {
          secretsOut[key] = strVal;
          yaml += `    ${key}: ${yamlQuote(`\${${key}}`)}\n`;
        } else {
          yaml += `    ${key}: ${yamlQuote(strVal)}\n`;
        }
      }
    }
    yaml += "\n";
