...
try { DEBUG_PANEL = globalThis.localStorage?.getItem("mycelium:debug:panel") === "1"; } catch { /* SSR/test */ }
const debugPanel = (...args: unknown[]) => { if (DEBUG_PANEL) console.log("[panel]", ...args); };
...
export function PluginDetailPanel({ plugin, onClose, onTogglePlugin, onToggleItem, onRemoveItem }: PluginDetailPanelProps) {
  ...
  }, [plugin]); // eslint-disable-line react-hooks/exhaustive-deps

  const handleAnimationEnd = useCallback((e: React.AnimationEvent) => {
    if (e.target !== e.currentTarget) return;
    debugPanel("animEnd", { animState, anim: e.animationName });
    if (animState === "opening") setAnimState("open");
    else if (animState === "closing") {
      setAnimState("closed");
      displayPlugin.current = null;
    }
  }, [animState]);

  const handleClose = useCallback(() => {
    debugPanel("handleClose", { animState });
    setAnimState("closing");
    onClose(); // immediately clear selectedPlugin to prevent stale async re-opens
  }, [onClose, animState]);

  if (animState === "closed") return null;
  const dp = displayPlugin.current;
  if (!dp) return null;
  ...
  const sections: Array<{ type: string; items: string[] }> = [
    { type: "skill", items: dp.skills },
    { type: "agent", items: dp.agents },
    { type: "command", items: dp.commands },
    { type: "hook", items: dp.hooks ?? [] },
    { type: "lib", items: dp.libs ?? [] },
  ...
  return (
    <>
      ...
      <div
        ...
        aria-label={dp.name}
        className={cn(
          "fixed right-0 top-0 z-50 h-full w-96 overflow-y-auto border-l bg-card text-card-foreground shadow-xl",
      ...
      >
        ...
        <div className="flex items-start justify-between border-b p-4">
          <div>
            <h2 className="text-lg font-bold">{dp.name}</h2>
            <div className="mt-1 flex items-center gap-2">
              {dp.version && <span className="text-sm text-muted-foreground">v{dp.version}</span>}
              <span className="rounded-full bg-teal-500/10 px-2 py-0.5 text-xs text-teal-400 border border-teal-500/30">
                {dp.marketplace}
              </span>
            </div>
            {dp.author && (
              <p className="mt-1 text-xs text-muted-foreground">by {dp.author}</p>
            )}
        ...
        {onRemoveItem && (
          <div className="p-4">
            ...
            >
              Remove {dp.marketplace === "system" ? "MCP" : dp.marketplace === "standalone" ? "Skill" : "Plugin"}
            </button>
      ...
      {confirmRemove && (
        <>
          ...
          <div className="fixed left-1/2 top-1/2 z-[70] w-80 -translate-x-1/2 -translate-y-1/2 rounded-lg border bg-card p-6 shadow-2xl">
            <h3 className="text-lg font-bold text-destructive">Remove {dp.name}?</h3>
            <p className="mt-2 text-sm text-muted-foreground">
              This will remove all components ({totalComponents} total) from ~/.mycelium.
              Run <code className="bg-muted px-1 rounded text-xs">mycelium sync</code> after to propagate.
            </p>
            <div className="mt-4 flex gap-2">
              <button
                onClick={() => {
                  const type = dp.marketplace === "system" ? "mcp" as const
                    : dp.marketplace === "standalone" ? "skill" as const
                    : "plugin" as const;
                  onRemoveItem?.(type, dp.name);
